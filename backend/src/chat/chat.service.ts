...
export class ChatService {
  ...
  async sendMessage(dto: SendMessageDto) {
    ...
    await firstValueFrom(this.httpService.post(`${this.peerUrl}`, dataToSend));
  ...
  getMessages(
    userId: string,
  ): { decrypted: string; from: string; timestamp: number }[] {
  ...
  private async fetchPublicKeyFromPeer(userId: string): Promise<string> {
    ...
    try {
      ...
      this.logger.error(
        `No se pudo obtener la clave pública de ${userId}`,
        err,
      );
